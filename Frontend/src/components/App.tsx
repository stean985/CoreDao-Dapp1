...
function App() {
  ...
  const checkWalletIsConnected = async () => {
    ...
    if (!ethereum) {
      ...
      console.log("Le Wallet existe! nous pouvons continuer!")
    }
    ...
    if (accounts.length !== 0) {
      ...
      console.log('Nous avons Trouvé un compte autorisé: ', account)
  ...
  const connectWalletHandler = async () => {
    ...
    try {
      ...
      console.log('Compte trouvé! Addresse: ', accounts[0])
  ...
  const store = async () => {
    try {
      ...
      if (ethereum) {
        ...
        console.log('écrire dans le contract')
        const tx = await storageContract.store(storeNumber)

        console.log('en attente de confirmatioon de la transaction')
        await tx.wait()

        console.log(
          `Transaction confirmée: https://scan.test.btcs.network/tx/${tx.hash}`
        )
      } else {
    ...
    return (
      <div>
        <p className="text-xl text-gray-400">
          Cliquer "Ecrire" ou "Lire" pour appeler le contrat intelligent
        </p>
        ...
        <div className="mt-8 text-center">
          <span className="text-mm">addresse de Contrat:</span>
          <a
            target="_blank"
  ...
  return (
    <div className="bg-white">
      ...
      <div className="mt-8 text-center">
        <div>
          <span className="text-mm">Alimentez votre compte:</span>
          <a
            target="_blank"
            className="ml-2 text-mm  text-orange-400 hover:text-orange-600"
        ...
        <div>
          <span className="text-mm">Comment se connecter</span>
          <a
            target="_blank"
            className="ml-2 text-mm  text-orange-400 hover:text-orange-600"
